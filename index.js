...
async function loadHomeTrips() {
  const todayTripList =
    document.getElementById("today-trip-list");

  const todayDescentList =
    document.getElementById("today-descent-list");

  if (!todayTripList || !todayDescentList) {
    console.error(
  ...
  try {
    ...
    for (const trip of trips) {
      ...
      const leaderName =
        await getLeaderName(trip.id, memberNames);
...
function createTodayTripCard(trip) {
  const card =
...
function createTodayDescentCard(trip) {
  const card =
    document.createElement("article");

  card.className = "trip-card descent";
  ...
  card.innerHTML = `
    ...
    <div class="status-row">
      ...
      <span class="status-badge status-descent">
        下山予定
      </span>
